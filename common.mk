...
# Prebuilt kernel modules (installed via copy files to keep them out of
# soong analysis; depmod metadata was generated by a source build and is
# frozen together with the modules. modules.load* files are generated by
# the build system itself from the *_LOAD lists in BoardConfigCommon.mk)
SM8550_PREBUILT_KERNEL_DIR := device/xiaomi/sm8550-common/prebuilt/kernel
PRODUCT_COPY_FILES += \
    $(foreach f,$(wildcard $(SM8550_PREBUILT_KERNEL_DIR)/vendor_dlkm/*.ko),\
        $(f):$(TARGET_COPY_OUT_VENDOR_DLKM)/lib/modules/$(notdir $(f))) \
    $(foreach f,$(wildcard $(SM8550_PREBUILT_KERNEL_DIR)/vendor_dlkm/modules.alias),\
        $(f):$(TARGET_COPY_OUT_VENDOR_DLKM)/lib/modules/$(notdir $(f))) \
    $(foreach f,$(wildcard $(SM8550_PREBUILT_KERNEL_DIR)/vendor_dlkm/modules.dep),\
        $(f):$(TARGET_COPY_OUT_VENDOR_DLKM)/lib/modules/$(notdir $(f))) \
    $(foreach f,$(wildcard $(SM8550_PREBUILT_KERNEL_DIR)/vendor_dlkm/modules.softdep),\
        $(f):$(TARGET_COPY_OUT_VENDOR_DLKM)/lib/modules/$(notdir $(f)))

PRODUCT_COPY_FILES += \
    $(foreach f,$(wildcard $(SM8550_PREBUILT_KERNEL_DIR)/ramdisk/*.ko),\
        $(f):$(TARGET_COPY_OUT_VENDOR_RAMDISK)/lib/modules/$(notdir $(f))) \
    $(foreach f,$(wildcard $(SM8550_PREBUILT_KERNEL_DIR)/ramdisk/modules.alias),\
        $(f):$(TARGET_COPY_OUT_VENDOR_RAMDISK)/lib/modules/$(notdir $(f))) \
    $(foreach f,$(wildcard $(SM8550_PREBUILT_KERNEL_DIR)/ramdisk/modules.dep),\
        $(f):$(TARGET_COPY_OUT_VENDOR_RAMDISK)/lib/modules/$(notdir $(f))) \
    $(foreach f,$(wildcard $(SM8550_PREBUILT_KERNEL_DIR)/ramdisk/modules.softdep),\
        $(f):$(TARGET_COPY_OUT_VENDOR_RAMDISK)/lib/modules/$(notdir $(f)))
